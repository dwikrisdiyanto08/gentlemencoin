...
#include "ui_zgentcontroldialog.h"
...
std::list<std::string> ZGentControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGentControlDialog::listMints;

ZGentControlDialog::ZGentControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGentControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGentControlDialog::~ZGentControlDialog()
{
    delete ui;
}

void ZGentControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGentControlDialog::updateList()
{
...
void ZGentControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGentControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGent_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGentControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGentControlDialog::GetSelectedMints()
{
...
void ZGentControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
