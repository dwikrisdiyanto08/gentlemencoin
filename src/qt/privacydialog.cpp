                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGENT ought to be enough for anybody." - Bill Gates, 2017
    ui->zGENTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGENTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGENT->setEnabled(false);
        ui->pushButtonMintzGENT->setToolTip(tr("zGENT is currently disabled due to maintenance."));

        ui->pushButtonSpendzGENT->setEnabled(false);
        ui->pushButtonSpendzGENT->setToolTip(tr("zGENT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGENTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGENT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGENT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGENT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGENT in ") +
...
void PrivacyDialog::on_pushButtonSpendzGENT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGENT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGENT
        sendzGENT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGENT
    sendzGENT();
}

void PrivacyDialog::on_pushButtonZGentControl_clicked()
{
    ZGentControlDialog* zGentControl = new ZGentControlDialog(this);
    zGentControl->setModel(walletModel);
    zGentControl->exec();
}

void PrivacyDialog::setZGentControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGENTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGENT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Gents Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGENTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGENT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GENT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGENT</b>";
    ...
    // use mints from zGent selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGentControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGentControlDialog::GetSelectedMints();
    }
    ...
    // Clear zgent selector in case it was used
    ZGentControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGent Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gent, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGent Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGENT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGENT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGENT "));
    ui->labelzGENTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGENTSyncStatus->setVisible(fShow);
}
