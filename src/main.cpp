...
#if defined(NDEBUG)
#error "Gents cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGent spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zGent spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zGent spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
     ...
     if (nHeight == 0) {
         nSubsidy = 450000 * COIN;
     } else if (nHeight <= 200) {
         nSubsidy = 20 * COIN;
     } else if (nHeight <= 21600) {
         nSubsidy = 20 * COIN * 0.9;
     } else if (nHeight <= 43200) {
         nSubsidy = 18 * COIN * 0.9;
     } else if (nHeight <= 64800) {
         nSubsidy = 16 * COIN * 0.9;
     } else if (nHeight <= 86400) {
         nSubsidy = 14 * COIN * 0.9;
     } else if (nHeight <= 108000) {
         nSubsidy = 12 * COIN * 0.9;
     } else if (nHeight <= Params().LAST_POW_BLOCK()) {
         nSubsidy = 10 * COIN * 0.9;
     } else if (nHeight <= 151200) {
         nSubsidy = 8 * COIN * 0.9;
     } else if (nHeight <= 172800) {
         nSubsidy = 6 * COIN * 0.9;
     } else if (nHeight <= 194400) {
         nSubsidy = 4 * COIN * 0.9;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Gents
...
{
    RenameThread("gents-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGENTMinted()
{
...
void RecalculateZGENTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGENT supply
...
bool RecalculateGENTSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zGent with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGentSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GENT to zGENT
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zGent spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zGent serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGent mints and %d zGent spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Gents: We use certain sporks during IBD, so check to see if they are
